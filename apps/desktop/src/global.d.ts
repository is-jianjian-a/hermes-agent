...
declare global {
  interface Window {
    hermesDesktop: {
      ...
      openSessionWindow: (sessionId: string, profile?: string | null) => Promise<{ ok: boolean; error?: string }>
      companion: {
        open: (mode: 'center' | 'island') => Promise<{ ok: boolean }>
        setMode: (mode: 'center' | 'expanded' | 'island') => Promise<{
          ok: boolean
          mode: 'center' | 'expanded' | 'island'
        }>
        getState: () => Promise<{
          enabled: boolean
        ...
        getDisplays: () => Promise<
          Array<{ id: string; name: string; hasNotch: boolean; notchHeight: number; notchWidth: number }>
        >
        setDisplay: (displayId: string) => Promise<{ ok: boolean; displayId: string }>
        disable: () => Promise<{ ok: boolean }>
        showContextMenu: () => Promise<{ ok: boolean }>
        onMode: (callback: (mode: 'center' | 'expanded' | 'island') => void) => () => void
      }
