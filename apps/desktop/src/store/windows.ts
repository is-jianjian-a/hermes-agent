...
export async function openSessionInNewWindow(sessionId: string, profile?: string | null): Promise<void> {
  if (!sessionId || !canOpenSessionWindow()) {
    return
  }

  try {
    const result = await window.hermesDesktop.openSessionWindow(sessionId, profile)
