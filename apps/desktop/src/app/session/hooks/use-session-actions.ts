...
export function useSessionActions({
  ...
  const resumeSession = useCallback(
    async (storedSessionId: string, replaceRoute = false) => {
      ...
      const requestedProfile = secondaryWindowProfile()
      const storedForProfile = $sessions
        .get()
        .find(
          session =>
            session.id === storedSessionId &&
            (!requestedProfile || normalizeProfileKey(session.profile) === normalizeProfileKey(requestedProfile))
        )
      const sessionProfile = storedForProfile?.profile || requestedProfile
      ...
      selectedStoredSessionIdRef.current = storedSessionId
      setSessionStartedAt(Date.now())
      const stored = storedForProfile || $sessions.get().find(session => session.id === storedSessionId)
