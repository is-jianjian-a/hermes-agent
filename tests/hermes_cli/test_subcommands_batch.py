...
def test_gui_builder_registers_companion_alias_surface():
    parser = argparse.ArgumentParser(prog="hermes")
    sub = parser.add_subparsers(dest="command")
    handler = _h("gui")
    build_gui_parser(sub, cmd_gui=handler)

    compact = parser.parse_args(["companion"])
    center = parser.parse_args(["companion", "--center"])

    assert compact.func is handler
    assert compact.companion_command is True
    assert compact.center is False
    assert center.center is True
