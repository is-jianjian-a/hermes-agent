...
class TestWebServerEndpoints:
    ...
    def test_session_summary_returns_only_compaction_message(self):
        from hermes_state import SessionDB

        db = SessionDB()
        try:
            db.create_session(session_id="summary-api", source="cli")
            db.append_message(session_id="summary-api", role="assistant", content="ordinary reply")
            db.append_message(
                session_id="summary-api",
                role="system",
        ...
        resp = self.client.get("/api/sessions/summary-api/summary")

        assert resp.status_code == 200
        assert resp.json()["summary"] == "API summary."

    def test_legacy_json_endpoints_exclude_database_sessions_and_delete(self):
        from hermes_constants import get_hermes_home
        from hermes_state import SessionDB
        ...
        (sessions_dir / "session_json-api.json").write_text(json.dumps(payload), encoding="utf-8")
        (sessions_dir / "session_modern-api.json").write_text(
            json.dumps({**payload, "session_id": "modern-api"}),
            encoding="utf-8",
        )
        db = SessionDB()
        try:
            db.create_session(session_id="modern-api", source="cli")
        finally:
            db.close()

        listed = self.client.get("/api/profiles/sessions/legacy")

        assert listed.status_code == 200
        assert [row["id"] for row in listed.json()["sessions"]] == ["json-api"]
        detail = self.client.get("/api/profiles/sessions/legacy/default/json-api")
        assert detail.status_code == 200
        assert detail.json()["messages"][0]["content"] == "legacy request"
        deleted = self.client.delete("/api/profiles/sessions/legacy/default/json-api")
        assert deleted.status_code == 200
        assert not (sessions_dir / "session_json-api.json").exists()
